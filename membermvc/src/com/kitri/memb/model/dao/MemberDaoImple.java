...
//			sql.append("or 		sigugun_building_name like '%'||?||'%' \n");
			...
			while (rs.next()) {
				...
				zipDto.setBuilding_number(rs.getString("building_number"));
				zipDto.setSigugun_building_name(rs.getString("sigugun_building_name"));
				list.add(zipDto);
			}
		} catch (SQLException e) {
			e.printStackTrace();
	...
	public int registerMember(MemberDetailDto memberDetailDto) {
		...
		try {
			...
			pstmt = conn.prepareStatement(sql.toString()); // 컴파일시 문법검사
	...
	public MemberDto loginMember(Map<String, String> map) {
		...
		PreparedStatement pstmt=null;
		ResultSet rs= null;
//		System.out.println("MemberDaoimple :" +map.get("userid")+" "+map.get("userpwd"));
		try {
